...
const User2 = () => {
  ...
  const Action = ({ row }) => {
    return (
      <div style={{ display: "flex" }}>
        ...
        <div style={{ color: "darkred", cursor: "pointer", marginLeft: "5px" }} onClick={()=>deleteSingleUser(row.row)}>
          <DeleteIcon />
        </div>
  ...
  return (
    ...
    <div className="flex " style={{justifyContent:"left",width:"80%",marginBottom:"20px"}}><div style={{width:"90%"}}><Button  variant="contained" style={{background: "#6750A4"}} onClick={()=>setOpenDialog({open:true,type:"add"})}>Add User</Button></div></div>
