...
import { addVillage,departmantList,editZone,getAllVilleges ,addUser,addAssignmentOfVillege,getTalukaList,addAssignmentOfDepartmsnt,removeAssignmentOfDepartmsnt,removeAssignmentOfVillege } from '../../Services/Apis/Api';
...
export default function VillageModel({action,dispatch,getDistrict,setAction,singleDistrict,setOpenAlert}) {
  ...
  const [checkedDepartments, setCheckedDepartments] = React.useState([]);
    const [checkedSelectedDepartments, setCheckedSelectedDepartments] = React.useState([]);
 ...
 console.log(zoneId,"zoneId")
  ...
  const closeDialog = ()=>{
   ...
   async function addScheme(){
        ...
        var data = {blockUniqueId:blockUniqueId,villageUniqueId:singleDistrict.villageUniqueId,villageName:userData}
        const response =  action.type=="edit"?await dispatch(editZone({data:data,id:zoneId})):await dispatch(addUser({...userData,EmpID:id}))
        ...
        setOpenAlert({
            open:true,
            mssg:action.type=="edit"?"User edit successfully":`user add successfully`,
            type:"success"
        })
...
dispatch(addAssignmentOfVillege({id:singleDistrict._id,data:mutiId})).then((res)=>{
  setFix(!fix)
})
setOpenAlert({
 open:true,
...
{
  try{
  let res = await dispatch(removeAssignmentOfDepartmsnt({id:singleDistrict._id,data:checkedSelectedDepartments}))
...
async function assignDepartment()
{
  try{
    ...
    let res = await dispatch(addAssignmentOfDepartmsnt({id:singleDistrict._id,data:checkedDepartments}))
...
await dispatch(removeAssignmentOfVillege({id:singleDistrict._id,data:selectedVillagesList}))
setOpenAlert({
 open:true,
  ...
  return (
    <div>
      ...
      setMultiId([])
        ...
        <DialogTitle>      
             {action.for!="Departmant"?action.for =="Village"?"Assign Village":action.type == "add"? "Add Block":"Edit Block":"Assign Departmant"}
        </DialogTitle>
        <DialogActions>
        </DialogActions>
        <DialogContent>
        {action.for!="Departmant"?action.for == "Village"?
        <div style={{display:"flex",textAlign:"center"}}>
          ...
          <SelecMultiSelectionBox reload={reload}  mutiId={mutiId} mutiId2={mutiId2} setMultiId={setMultiId} setMultiId2={setMultiId2} singleDistrict={singleDistrict} data={taluka} setCheckedVillageArray={setCheckedVillageArray} />
          <InnerComponent setSelectedVillage={setSelectedVillage} selectedVillagesList={selectedVillagesList} setSelectedVillagesList={setSelectedVillagesList} selectedVillage={selectedVillage} singleDistrict={singleDistrict}/>
          </div>    
        :<div>
          <TextField
            autoFocus
          ...
          <FormControl fullWidth>
            ...
            onChange={(e)=>setUserData({...userData,role:e.target.value})}
              ...
              >
                ...
                <MenuItem value="supdistrictManager">SupdistrictManager</MenuItem>
              </Select>
            </FormControl>
            </div>:  <div>
            ...
            <SelectMultiSelectionDepartment checkedSelectedDepartments={checkedSelectedDepartments} setCheckedSelectedDepartments={setCheckedSelectedDepartments} checkedDepartments={checkedDepartments} setCheckedDepartments={setCheckedDepartments} singleDistrict={singleDistrict}  selected={departmantSelected} allDepartmantData={allDepartmantData} setAllDepartmantData={setAllDepartmantData} setSelected={setDepartmantSelected} departmant={departmant} setDepartmant={setDepartmant}/>
        ...
        <DialogActions>
          ...
          {action.for!="Departmant"?action.for =="Village"?<div><Button onClick={()=>addVillegesAssigment()}>Assign</Button><Button onClick={()=>removeVillegesAssigment()}>Remove</Button></div>:<Button onClick={()=>addScheme()}>Add</Button>:<div><Button onClick={()=>assignDepartment()}>Assign</Button><Button onClick={()=>removeDepartment()}>remove</Button></div>}
        </DialogActions>
      </Dialog>
    </div>
  );
